...
public:
    StereoProjector(ros::NodeHandle& nh)
        : it_(nh),
          ...
          disparity_sub_(it_, "/zed/zed_node/disparity/disparity_image", 1),
          left_info_sub_(nh, "/zed/zed_node/left/camera_info", 1),
          right_info_sub_(nh, "/zed/zed_node/right/camera_info", 1),
...
private:
    ...
    image_transport::SubscriberFilter left_image_sub_, disparity_sub_;
    ...
    void callback(const sensor_msgs::ImageConstPtr& left_msg,
                  const sensor_msgs::ImageConstPtr& disp_msg,
                  const sensor_msgs::CameraInfoConstPtr& left_info,
                  const sensor_msgs::CameraInfoConstPtr& right_info)
    {
        // Convert images
        cv_bridge::CvImageConstPtr left_cv, disp_cv;
        try {
        ...
        int width = disp_cv->image.cols;
        int height = disp_cv->image.rows;
        ...
        for (int v = 0; v < height; v++) {
            for (int u = 0; u < width; u++) {
                ...
                float disp = disp_cv->image.at<float>(v, u);

                if (std::isnan(disp) || disp <= 0.1) {
                ...
                cv::Point3d pt;
                model.projectDisparityTo3d(cv::Point2d(u, v), disp, pt);
